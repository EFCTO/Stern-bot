...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("youtube")
    .setDescription("유튜브 채널 업로드 알림을 설정합니다.")
    .addSubcommand(sub =>
...
async function handleRegister(interaction, service) {
  ...
  try {
    ...
    if (!resolvedId) {
      await interaction.editReply("유효한 유튜브 채널을 찾을 수 없습니다. 채널 ID, URL 또는 @핸들을 입력해주세요.");
      return;
    }
    ...
    const lines = [
      `유튜브 채널 **${channel.channelTitle}**(ID: \`${channel.channelId}\`)이 등록되었습니다.`,
      `알림 채널: <#${targetChannel.id}>`
    ];

    if (channel.lastVideoId) {
      lines.push("기존 영상 이후 새 영상이 올라오면 알림이 전송됩니다.");
    }

    await interaction.editReply(lines.join("\n"));
  } catch (error) {
    console.error("유튜브 채널 등록 실패", error);
    await interaction.editReply("유튜브 정보를 불러오는 중 문제가 발생했습니다. 잠시 후 다시 시도해주세요.");
  }
}
...
async function handleClear(interaction, service) {
  ...
  const channel = service.getChannel();
  if (!channel) {
    await interaction.editReply("등록된 유튜브 채널이 없습니다.");
    return;
  }

  await service.clearChannel();
  await interaction.editReply(`유튜브 채널 **${channel.channelTitle}** 등록을 해제했습니다.`);
}
